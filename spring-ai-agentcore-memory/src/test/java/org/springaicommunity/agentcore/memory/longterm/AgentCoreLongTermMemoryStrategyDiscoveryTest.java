...
class AgentCoreLongTermMemoryStrategyDiscoveryTest {
	...
	void shouldExtractReflectionsNamespacesForEpisodicStrategy() {
		...
		mockGetMemory(List.of(strategy));

		List<DiscoveredStrategy> discovered = new AgentCoreLongTermMemoryStrategyDiscovery(controlClient)
	...
	void shouldReturnEmptyReflectionsForEpisodicWithoutReflectionConfig() {
		...
		DiscoveredStrategy ds = new AgentCoreLongTermMemoryStrategyDiscovery(controlClient).discoverStrategies("mem-1")
			.get(0);
