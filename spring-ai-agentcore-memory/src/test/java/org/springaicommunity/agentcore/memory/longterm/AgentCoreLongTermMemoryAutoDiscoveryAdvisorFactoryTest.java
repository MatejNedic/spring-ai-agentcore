...
class AgentCoreLongTermMemoryAutoDiscoveryAdvisorFactoryTest {
	...
	void episodicWithDiscoveredReflectionsNoExplicitConfig() {
		...
		var advisor = factory(config).createAdvisors(List.of(discovered)).get(0);
		stubEmptyMemories();
		advisor.adviseCall(userRequest("how is the weather?"), chain);

		// Then: reflections retrieval uses the discovered namespace under the same
		// strategy.
		verify(retriever).searchMemories(eq(STRATEGY_ID), anyString(), anyString(), eq("how is the weather?"), anyInt(),
	...
	void episodicExplicitReflectionsNamespacePatternWinsOverDiscovered() {
		...
		var advisor = factory(config).createAdvisors(List.of(discovered)).get(0);
		stubEmptyMemories();
		advisor.adviseCall(userRequest("q"), chain);

		// Then: reflections retrieval uses the explicit override, not the discovered one.
		verify(retriever).searchMemories(eq(STRATEGY_ID), anyString(), anyString(), eq("q"), anyInt(),
				eq(explicitReflections));
		verify(retriever, never()).searchMemories(eq(STRATEGY_ID), anyString(), anyString(), eq("q"), anyInt(),
				eq(DISCOVERED_REFLECTIONS_NS));
	}
	...
	void episodicExplicitLegacyReflectionsStrategyIdIsUsedWhenNoModernOverride() {
		...
		verify(retriever).searchMemories(eq(legacyReflectionsStrategy), anyString(), anyString(), eq("q"), anyInt(),
				eq(DISCOVERED_EPISODES_NS));
	}
	...
	void episodicExplicitModernReflectionsBeatsLegacyWhenBothPresent() {
		...
		verify(retriever).searchMemories(eq(STRATEGY_ID), anyString(), anyString(), eq("q"), anyInt(), eq(modern));
		verify(retriever, never()).searchMemories(eq(legacy), anyString(), anyString(), anyString(), anyInt(),
				anyString());
	}
	...
	void episodicWithNoReflectionsAnywhereRunsEpisodesOnly() {
		...
		verify(retriever).searchMemories(eq(STRATEGY_ID), anyString(), anyString(), eq("q"), anyInt(),
				eq(DISCOVERED_EPISODES_NS));
		verify(retriever, org.mockito.Mockito.times(1)).searchMemories(anyString(), anyString(), anyString(),
	...
	void semanticStrategyIsBuiltWithDiscoveredNamespace() {
		...
		verify(retriever).searchMemories(eq("sem-1"), anyString(), anyString(), eq("q"), anyInt(),
				eq(DISCOVERED_EPISODES_NS));
	}
	...
	private AgentCoreLongTermMemoryAutoDiscoveryAdvisorFactory factory(AgentCoreLongTermMemoryProperties config) {
		return new AgentCoreLongTermMemoryAutoDiscoveryAdvisorFactory(retriever, config, MEMORY_ID, null);
	}
	...
	private void stubEmptyMemories() {
		org.mockito.Mockito.lenient()
			...
			.thenReturn(List.of());
		org.mockito.Mockito.lenient()
			.when(retriever.listMemories(anyString(), anyString(), anyString()))
			.thenReturn(List.of());
	}
